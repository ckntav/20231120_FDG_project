...
mapping_ensembl <- raw_mapping %>% 
  dplyr::select("UniProtKB_AC", "UniProtKB_ID", "Ensembl")

mapping_ensembl %>% group_by(Ensembl) %>% tally %>% arrange(desc(n))

saveRDS(mapping_ensembl, file = "output/rds/mapping_ensembl.rds")
